/*  
    NickelExtend
     ~ Simple JavaScript Framework for Chrome Only.
       Part of Nickel Framework for Pokki
       Inspired by MooTools and underscore.js
...
window.HTMLElement.prototype.getFirstLevelChildren = function getFirstLevelChildren(node)
{

    node       = node || '*';
    var els    = this.getElementsByTagName(node);
    var result = new Array(); 

    for (var i = 0; i < els.length; i++) { 
        if (els[i].parentNode === this) {
            result.push(els[i]);
        }
    }
...
window.HTMLElement.prototype.setStyle = function setStyle(key, value)
{
    ...
    if (typeof key == 'string') {
        var format = NickelExtendTools.formatStyleValue(key, value);
...
/*
* addDoubleClass: Add a class and a second one with a 10ms delay.
*
* @param String class1 Name of the CSS class to add.
* @param String class2 Name of the second CSS class to add.
*/
window.HTMLElement.prototype.addDoubleClass = function addDoubleClass(class1, class2)
{
